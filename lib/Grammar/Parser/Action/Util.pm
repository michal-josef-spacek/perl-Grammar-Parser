...
package Grammar::Parser::Action::Util v1.0.0 {
	...
	our $DEFAULT_ACTION_PREFIX = 'rule_';
	...
	sub rule_handler_alias {
		my (undef, $name, @list) = @_;
		my $value = _first_ast @list;

		return unless $value;
		return +{ $name => values %$value };
	}

	sub rule_handler_alias_merge {
		my (undef, $name, @list) = @_;
		my ($value, @rest) = _find_ast @list;

		return unless $value;
		return +{ $name => values %$value, map %$_, reverse @rest };
	}

	sub rule_handler_default {
		my (undef, $name, @list) = @_;

		return +{ $name => { map %$_, _find_ast reverse @list }};
	}

	sub rule_handler_literal {
		return;
	}

	sub rule_handler_literal_value {
	...
	sub rule_handler_list {
		my (undef, $name, @list) = @_;

		return +{ $name => [
			map exists $_->{$name} ? @{ $_->{$name} } : $_, _find_ast @list
		] };
	}

	sub rule_handler_merge {
		my (undef, $name, @list) = @_;

		return +{ map %$_, reverse _find_ast @list };
	}

	sub rule_handler_pass_through {
		my (undef, $name, @def) = @_;

		my $retval = _first_ast @def;
		$retval //= $def[0];

		return $retval;
	}

	sub rule_handler_symbol {
		my (undef, $name) = @_;

		return +{ $name => $name };
	}

	sub rule_handler_value {
		my (undef, $name, @list) = @_;
	...
	sub generate_rule_handler {
		...
		my $template = 'rule_handler_' . $type;

		# TODO: throw an exception
		die "Unknown handler '$type'\n"
			unless __PACKAGE__->can ($template);

		return eval "sub { goto \\&$template }";
	}

	sub install_rule {
		my ($prefix, $name, $code, @params) = @_;
		...
		unless (is_coderef $code) {
			$code = generate_rule_handler ($code, @params);
	...
	sub _exporter_expand_sub {
		...
		my $action_prefix = undef
			// $args->{action_prefix}
			// $globals->{action_prefix}
			// $DEFAULT_ACTION_PREFIX
			;

		my $as = $args->{as};
		$as //= $export_name if $name eq 'rule';
		$as //= "$action_prefix$export_name";

		my $code;
		$code //= sub { install_rule $action_prefix, @_ }
			if $name eq 'rule';

		$code //= sub { $action_prefix . $_[0] }
			if $name eq 'action_name';

		$code //= generate_rule_handler $name;

		Sub::Name::subname "$globals->{into}::${as}" => $code;
